  ...
  $.ajax({
    ...
    dataType: 'json',
      success: function(test_dic) {
        ...
        for (i=1;i<=n;i++){
          item = '<h3>'+test_dic[i-1].vote_name+'<b style=font-size:15px>&emsp;<span id=person'+i+'></span></h3><span id=T'+i+'_option0></span></div><div><span id=T'+i+'_option1></span></div><div><span id=T'+i+'_option2></span><br /><br />';
          console.log(item)
